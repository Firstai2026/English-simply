...
export function MatchChoice({ pool, onExit }) {
  ...
  function playCurrent() {
    if (current && current.direction === 'en') {
  ...
  const promptText = current.direction === 'en' ? current.card.front : current.card.back;

  return (
    <div className="px-5 pb-8 flex flex-col items-center" style={{ minHeight: '60vh' }}>
      <p className="text-sm mb-5" style={{ color: 'var(--ink-soft)' }}>{doneCount + 1} / {pool.length}</p>
      <div
        onClick={playCurrent}
        className={`dc-surface p-6 mb-6 text-center ${current.direction === 'en' ? 'dc-tappable cursor-pointer' : ''}`}
        style={{ width: '100%', maxWidth: 320 }}
      >
        {current.direction === 'en' && current.card.hasImage && mediaCache[current.card.id] && mediaCache[current.card.id].image && (
          <img
            src={mediaCache[current.card.id].image}
            alt=""
            className="rounded-2xl mb-3"
            style={{ width: '100%', maxHeight: 140, objectFit: 'cover' }}
          />
        )}
        <p className="dc-display font-semibold" style={{ fontSize: 26 }}>{promptText}</p>
        {current.direction === 'en' && (
          <Icon name="volume2" size={16} style={{ color: 'var(--accent)', marginTop: 8 }} />
        )}
      </div>
